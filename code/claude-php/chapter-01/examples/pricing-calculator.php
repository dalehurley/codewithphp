...
function displayCostComparison(array $scenarios): void
{
    ...
    foreach ($scenarios as $name => $tokens) {
        ...
        foreach (['claude-haiku-4-5', 'claude-sonnet-4-5', 'claude-opus-4-1'] as $model) {
...
try {
    ...
    foreach ($scenarios as $scenario) {
        $opusCost = calculateCost('claude-opus-4-1', $scenario['input'], $scenario['output']) * $scenario['volume'];
        $sonnetCost = calculateCost('claude-sonnet-4-5', $scenario['input'], $scenario['output']) * $scenario['volume'];
